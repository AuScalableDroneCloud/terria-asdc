...
import Constructor from "terriajs/lib/Core/Constructor";
import filterOutUndefined from "terriajs/lib/Core/filterOutUndefined";
import BoxDrawing from "terriajs/lib/Models/BoxDrawing";
...
import ClippingPlanesTraits from "terriajs/lib/Traits/TraitsClasses/ClippingPlanesTraits";
import HeadingPitchRollTraits from "terriajs/lib/Traits/TraitsClasses/HeadingPitchRollTraits";
...
function ClippingMixin<T extends Constructor<BaseType>>(
  ...
  abstract class MixedClass extends Base implements InstanceType {
    ...
    get selectableDimensions(): SelectableDimension[] {
      ...
      if (this.uniqueId?.startsWith("//WebODM Projects") || this.uniqueId?.startsWith("//Shared WebODM Datasets")){
        sd[sd.length-1].selectableDimensions.push({
          type: "button",
          id: `export`,
          value: "Export",
          setDimensionValue: stratumId => {
            ...
            groundScalePoints.map((entity,index)=>{
                var pos = Cartographic.fromCartesian(entity.position.getValue(JulianDate.now()));
                var lon = pos.longitude * CesiumMath.DEGREES_PER_RADIAN;
                var lat = pos.latitude * CesiumMath.DEGREES_PER_RADIAN;
                wktPolygon += `${lon} ${lat}`;
            ...
            var heights = scalePoints.map(entity =>{
              var pos = Cartographic.fromCartesian(entity.position.getValue(now));
              return pos.height;
            });
            ...
            var lons=scalePoints.map(entity =>{
              var pos = Cartographic.fromCartesian(entity.position.getValue(now));
              return pos.longitude* CesiumMath.DEGREES_PER_RADIAN;
            });
            ...
            var lats=scalePoints.map(entity =>{
              var pos = Cartographic.fromCartesian(entity.position.getValue(now));
              return pos.latitude* CesiumMath.DEGREES_PER_RADIAN;
            });
            ...
            const outside = !this.clippingPlaneCollection?.unionClippingRegions;
